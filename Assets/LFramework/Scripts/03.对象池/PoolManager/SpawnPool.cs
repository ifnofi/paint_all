...
namespace PathologicalGames
{
    ...
    public class PrefabPool
    {
        ...
        /// 
        /// 打开此选项以激活此池的筛选功能。如果池的大小变得太大，可以使用此特性从池中删除废弃(非活动)实例。
        /// 除非您需要管理内存问题，否则不要使用它!这只应该在极端情况下用于内存管理。对于大多数池(或游戏)，
        /// 最好不要设置此选项，因为内存比性能更丰富。如果您确实需要此功能，则可以微调触发此功能的频率，
        /// 以针对极端事件。使用这种方法的一个很好的例子是，如果你正在集中炮弹，通常一次不需要超过10个，
        /// 但在一次大型的一次性交火中需要50个炮弹。而不是从那时起在内存中保留额外的40个，
        /// 将“Cull Above”属性设置为15(远高于预期的最大值)，池将从游戏中销毁()额外的实例以释放内存。
        /// 这不会立即完成，因为您不希望这个淘汰特性与池进行战斗，并在战斗仍在进行时导致额外的Instantiate()和Destroy()调用。
        /// 有关如何对其进行微调的更多信息，请参阅“选择延迟”。
        /// </summary>
        public bool cullDespawned = false;

        /// <summary>
        /// The number of TOTAL (spawned + despawned) instances to keep.
        /// 要保留的TOTAL(已生成+已废弃)实例的数量。
        /// </summary>
        ...
        /// 
        /// 在剔除之前等待的时间，以秒为单位。这是定时的从队列的总数(生成+放弃)的那一刻起，
        /// 直到计数低于“Cull Above”。变得比'Cull Above'更大。一旦触发，计时器就会重复
        /// </summary>
        ...
        /// here will be used
        ///
        /// 在运行时打印信息，使调试更容易。如果owner SpawnPool为true，则此设置将被设置为true，否则将使用用户在这里的设置
        /// </summary>
        ...
        /// 
        /// 在内部用来引用owner spawnPool，比如锚定协同例程。
        /// </summary>
