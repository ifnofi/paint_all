        ...
        public static void Generate(CodeGenTask task)
        {
            ...
            root.Namespace(namespaceName, ns =>
            {
                ...
                ns.Class(task.ClassName, parentClassName, true, false, cs =>
                {
                    ...
                    {
                        cs.CustomScope("public override IEnumerator InitPanel()", false, scope =>
                        {
                            scope.Custom("yield return base.InitPanel();");
                            scope.Custom("");
                        });
                        cs.CustomScope("public override void Open(float duration =0.5f,float delay =0f)", false, scope =>
                        {
                            scope.Custom("base.Open(duration,delay);");
                            scope.Custom("");
                        });
                        cs.CustomScope("public override void Close(float duration =0.5f,float delay =0f)", false, scope =>
                        {
                            scope.Custom("base.Close(duration,delay);");
                            scope.Custom("");
                        });
                    }
