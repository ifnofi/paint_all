namespace SRDebugger.Editor
{
    ...
    internal class SRDebuggerSettingsWindow : EditorWindow
    {
        ...
        private readonly string[] _tabs = Enum.GetNames(typeof(Tabs)).Select(s => s.Replace('_', ' ')).ToArray();
        ...
        private bool SettingsReady(out Settings settings)
        {
            ...
            {
                var result =
                    SRInternalEditorUtil.EditorSettings.TryGetOrCreate(out settings, out var message);
                    ...
                    case SRInternalEditorUtil.SettingsResult.Loaded:
                        {
                            ...
                            this._currentEntryCode = this.GetEntryCodeString(settings);

                            if (string.IsNullOrEmpty(settings.ApiKey))
                            {
                                this._showBugReportSignupForm = true;
                            }
            ...
            GUILayout.Label(this._error, SRInternalEditorUtil.Styles.ParagraphLabel);
        ...
        private void OnGUI()
        {
            ...
            if (!this.SettingsReady(out var settings))
            {
                return;
            }
            ...
            {
                var xStart = Mathf.RoundToInt(i * rect.width / this._tabs.Length);
                var xEnd = Mathf.RoundToInt((i + 1) * rect.width / this._tabs.Length);

                var pos = new Rect(rect.x + xStart, rect.y, xEnd - xStart, height);

                if (GUI.Toggle(pos, (int)this._selectedTab == i, new GUIContent(this._tabs[i]), EditorStyles.toolbarButton))
                {
                    this._selectedTab = (Tabs)i;
                }
            }
            ...
            {
                case Tabs.General:
                    this.DrawTabGeneral(settings);
                    break;

                case Tabs.Layout:
                    this.DrawTabLayout(settings);
                    break;

                case Tabs.Bug_Reporter:
                    this.DrawTabBugReporter(settings);
                    break;

                case Tabs.Shortcuts:
                    this.DrawTabShortcuts(settings);
                    break;

                case Tabs.Advanced:
                    this.DrawTabAdvanced(settings);
                    break;
            }
            ...
            SRInternalEditorUtil.DrawFooterLayout(this.position.width);
        ...
        private void DrawTabGeneral(Settings settings)
        {
            ...
            var newCode = EditorGUILayout.TextField("Entry Code", this._currentEntryCode);

            if (newCode != this._currentEntryCode)
            {
                ...
                if (newCode.Length == 4)
                {
                    this.UpdateEntryCode(settings, newCode);
                }
            }
        ...
        {
            if (this._showBugReportSignupForm)
            {
                this.DrawBugReportSignupForm(settings);
                return;
            }
            ...
            if (!string.IsNullOrEmpty(settings.ApiKey))
            {
                ...
                this._enableButton = EditorGUILayout.Toggle("Enable Button", this._enableButton, GUILayout.ExpandWidth(false));

                EditorGUI.BeginDisabledGroup(!this._enableButton);
        ...
        private void DrawBugReportSignupForm(Settings settings)
        {
            ...
            EditorGUI.BeginDisabledGroup(isWeb || !this._enableTabChange);
            ...
            {
                this._showBugReportSignupForm = false;
                this.Repaint();
            }
            ...
            this._invoiceNumber = EditorGUILayout.TextField(this._invoiceNumber);
            ...
            this._emailAddress = EditorGUILayout.TextField(this._emailAddress);
            ...
            this._agreeLegal = EditorGUILayout.Toggle(this._agreeLegal);

            EditorGUILayout.EndHorizontal();

            EditorGUILayout.Space();

            var isEnabled = !string.IsNullOrEmpty(this._invoiceNumber) && !string.IsNullOrEmpty(this._emailAddress) && this._agreeLegal;
            EditorGUI.BeginDisabledGroup(!isEnabled);

            if (GUILayout.Button("Submit"))
            {
                this._errorMessage = null;
                this._enableTabChange = false;

                EditorApplication.delayCall += () =>
                {
                    ApiSignup.SignUp(this._emailAddress, this._invoiceNumber,
                        (success, key, email, error) => this.OnSignupResult(success, key, email, error, settings));
                    this.Repaint();
                };
            }
            ...
            {
                EditorGUILayout.HelpBox(this._errorMessage, MessageType.Error, true);
            }
        ...
        private void OnSignupResult(bool didSucceed, string apiKey, string email, string error, Settings settings)
        {
            ...
            this._selectedTab = Tabs.Bug_Reporter;
        ...
        {
            if (this._keyboardShortcutList == null)
            {
                this._keyboardShortcutList = new ReorderableList((IList)settings.KeyboardShortcuts,
                    typeof(Settings.KeyboardShortcut), false, true, true, true);
                this._keyboardShortcutList.drawHeaderCallback = this.DrawKeyboardListHeaderCallback;
                this._keyboardShortcutList.drawElementCallback = this.DrawKeyboardListItemCallback;
                this._keyboardShortcutList.onAddCallback += this.OnAddKeyboardListCallback;
                this._keyboardShortcutList.onRemoveCallback += this.OnRemoveKeyboardListCallback;
            }
            ...
            var dupe = this.DetectDuplicateKeyboardShortcuts(settings);
            ...
            this._scrollPosition = EditorGUILayout.BeginScrollView(this._scrollPosition, false, false,
                GUILayout.Width(this.position.width - 11));

            EditorGUILayout.BeginVertical(GUILayout.Width(this.position.width - 30));

            this._keyboardShortcutList.DoLayoutList();
        ...
        {
            this._scrollPosition = EditorGUILayout.BeginScrollView(this._scrollPosition, false, true);
