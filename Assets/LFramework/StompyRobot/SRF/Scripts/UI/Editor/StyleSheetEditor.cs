...
namespace SRF.UI.Editor
{
    ...
    public class StyleSheetEditor : UnityEditor.Editor
    {
        ...
        {
            var styleSheet = this.target as StyleSheet;

            if (styleSheet == null)
            {
                Debug.LogWarning("Expected target to be StyleSheer", this.target);
                return;
            }
            ...
            this._newKeyField = EditorGUILayout.TextField(this._newKeyField);

            GUI.enabled = !string.IsNullOrEmpty(this._newKeyField) && styleSheet.GetStyle(this._newKeyField) == null;

            if (GUILayout.Button("Add"))
            {
                Undo.RecordObject(styleSheet, "Add Style");
                styleSheet.AddStyle(this._newKeyField);
                EditorUtility.SetDirty(styleSheet);
